...
class UpdateServiceTests(unittest.TestCase):
    ...
    def _manifest(version: str) -> dict[str, object]:
        return {
            ...
            "download": {
                ...
                "mirrors": {
                    "github": "https://github.com/example/WycccModManager/releases/download/0.9.4/WycccModManager.exe",
                    "gitee": "https://gitee.com/example/WycccModManager/releases/download/0.9.4/WycccModManager.exe",
                },
                "sha256": "1" * 64,
                "size": 1234,
            },
    ...
    def test_local_manifest_check_download_hash_and_ignore_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            downloaded = service.download("0.9.4")
    ...
    def test_non_chinese_checks_github_first_but_uses_a_newer_gitee_release(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def read_manifest(url: str) -> tuple[dict[str, object], str]:
                calls.append(url)
                version = "0.5.0" if url == GITHUB_UPDATE_MANIFEST_URL else "0.9.4"
            ...
            self.assertEqual(checked["source"], "gitee")
            self.assertEqual(checked["version"], "0.9.4")
    ...
    def test_repository_check_falls_back_when_the_preferred_source_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def read_manifest(url: str) -> tuple[dict[str, object], str]:
                ...
                return self._manifest("0.9.4"), url
