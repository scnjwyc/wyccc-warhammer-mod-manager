...
class PackagedRuntimeTests(unittest.TestCase):
    ...
    def test_release_version_and_changelog_are_synchronized(self) -> None:
        ...
        self.assertIn("appVersion: '1.1.1'", frontend_store)
        self.assertIn("filevers=(1, 1, 1, 0)", version_info)
        self.assertIn("prodvers=(1, 1, 1, 0)", version_info)
        self.assertIn("StringStruct('ProductVersion', '1.1.1')", version_info)
        self.assertIn("`1.1.1`", readme)
        self.assertIn("`1.1.1`", readme_en)
        ...
        self.assertEqual(
            ...
            ["1.1.1", "1.1.0", "1.0.9", "1.0.8", "1.0.7", "1.0.6", "1.0.5", "1.0.1", "1.0.0", "0.9.9"],
        )
        self.assertEqual(changelog[7]["version"], "1.0.1")
        self.assertEqual(changelog[0]["entries"][0]["changes"][0]["text"], "适配战锤9.0版本更新")
