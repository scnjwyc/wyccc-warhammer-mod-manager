...
class PackagedRuntimeTests(unittest.TestCase):
    ...
    def test_release_version_and_changelog_are_synchronized(self) -> None:
        ...
        self.assertEqual(APP_VERSION, "1.0.9")
        self.assertEqual(project["project"]["version"], APP_VERSION)
        self.assertEqual(frontend["version"], APP_VERSION)
        self.assertIn("appVersion: '1.0.9'", frontend_store)
        self.assertIn("filevers=(1, 0, 9, 0)", version_info)
        self.assertIn("prodvers=(1, 0, 9, 0)", version_info)
        self.assertIn("StringStruct('ProductVersion', '1.0.9')", version_info)
        self.assertIn("`1.0.9`", readme)
        self.assertIn("`1.0.9`", readme_en)
        self.assertEqual(update_manifest["schema_version"], 1)
        self.assertEqual(update_manifest["app"], APP_NAME)
        self.assertEqual(update_manifest["version"], "1.0.9")
        ...
        self.assertTrue(update_manifest["download"]["url"].startswith("https://gitee.com/"))
        self.assertTrue(
            update_manifest["download"]["mirrors"]["gitee"].startswith("https://gitee.com/")
        )
        ...
        self.assertEqual(
            ...
            ["1.0.9", "1.0.8", "1.0.7", "1.0.6", "1.0.5", "1.0.1", "1.0.0", "0.9.9", "0.9.8", "0.9.7"],
        )
        self.assertEqual(changelog[6]["version"], "1.0.0")
    ...
    def test_085_changelog_describes_game_path_and_overhaul_fixes(self) -> None:
        ...
        structure_keys = {
            key
            for title_key, changes in next(
                item for item in CHANGELOG_STRUCTURE if item["version"] == "0.8.5"
            )["entries"]
            for key in (title_key, *(text_key for _change_type, text_key in changes))
        }
        self.assertSetEqual(structure_keys, expected_keys)
        ...
        for language in languages:
            release = next(
                item for item in get_all_changelogs(language) if item["version"] == "0.8.5"
            )
            self.assertEqual([len(entry["changes"]) for entry in release["entries"]], [3])

    def test_086_changelog_describes_playset_dependency_fix(self) -> None:
        languages = ("zh-CN", "en-US", "ko-KR", "ru-RU", "ja-JP", "es-ES")
        expected_keys = {
            "v086_fixed_title",
            "v086_playset_dependencies",
            "v086_settings_page_ui",
        }
        release_structure = next(
            item for item in CHANGELOG_STRUCTURE if item["version"] == "0.8.6"
        )
        structure_keys = {
            key
            for title_key, changes in release_structure["entries"]
        ...
        for language in languages:
            release = next(
                item for item in get_all_changelogs(language) if item["version"] == "0.8.6"
            )
            self.assertEqual(release["version"], "0.8.6")
            self.assertEqual([len(entry["changes"]) for entry in release["entries"]], [2])
        english = str(next(
            item for item in get_all_changelogs("en-US") if item["version"] == "0.8.6"
        ))
        self.assertIn("installed but not enabled", english)
        self.assertIn("Settings page", english)
