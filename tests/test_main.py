...
class PackagedRuntimeTests(unittest.TestCase):
    ...
    def test_release_version_and_changelog_are_synchronized(self) -> None:
        ...
        self.assertIn("appVersion: '1.0.5'", frontend_store)
        self.assertIn("filevers=(1, 0, 5, 0)", version_info)
        self.assertIn("StringStruct('ProductVersion', '1.0.5')", version_info)
        self.assertIn("`1.0.5`", readme)
        self.assertIn("`1.0.5`", readme_en)
        self.assertEqual(update_manifest["schema_version"], 1)
        self.assertEqual(update_manifest["app"], APP_NAME)
        self.assertEqual(update_manifest["version"], "1.0.5")
        ...
        self.assertEqual(
            ...
            ["1.0.5", "1.0.1", "1.0.0", "0.9.9", "0.9.8", "0.9.7", "0.9.6", "0.9.5", "0.9.4", "0.9.3"],
        )
        self.assertEqual(changelog[2]["version"], "1.0.0")
