...
from .start_options import (
    ...
    build_dynamic_ror_compatibility_patch,
    collect_game_data_source_snapshot,
)


DYNAMIC_ROR_COMPATIBILITY_MANIFEST_NAME = (
    "!!!!wyccc_dynamic_ror_compatibility.json"
)
FINGERPRINT_SCHEMA_VERSION = 1
DYNAMIC_ROR_COMPATIBILITY_BUILDER_VERSION = 1
SETTING_KEY = "dynamic_ror_compatibility_patch_enabled"
...
def dynamic_ror_compatibility_requested(settings: Mapping[str, Any]) -> bool:
    return _coerce_bool(settings.get(SETTING_KEY, False))
...
def build_dynamic_ror_compatibility_inputs(
    data_path: str | Path,
    ...
    enabled = dynamic_ror_compatibility_requested(settings)
    ...
    return {
        ...
        "builder_version": DYNAMIC_ROR_COMPATIBILITY_BUILDER_VERSION,
...
def fingerprint_dynamic_ror_compatibility_inputs(inputs: Mapping[str, Any]) -> str:
    encoded = json.dumps(
        inputs,
        ensure_ascii=False,
        sort_keys=True,
...
def _manifest_store(output_dir: Path) -> AtomicJsonStore:
    return AtomicJsonStore(
        output_dir / DYNAMIC_ROR_COMPATIBILITY_MANIFEST_NAME,
        lambda: {},
    )
...
def _output_record(path: Path) -> dict[str, Any]:
    stat = path.stat()
    return {
        "name": DYNAMIC_ROR_COMPATIBILITY_PATCH_NAME,
        "size": stat.st_size,
...
def _manifest_output_is_valid(manifest: Mapping[str, Any], output_dir: Path) -> bool:
    patch_path = output_dir / DYNAMIC_ROR_COMPATIBILITY_PATCH_NAME
    ...
    if not isinstance(output, Mapping):
        return False
    if output.get("name") != DYNAMIC_ROR_COMPATIBILITY_PATCH_NAME:
        return False
    try:
...
def _result_from_manifest(
    ...
    if manifest.get("build_status") == "generated":
        path = str(
            (output_dir / DYNAMIC_ROR_COMPATIBILITY_PATCH_NAME).resolve(
                strict=False
            )
        )
...
def ensure_dynamic_ror_compatibility_patch(
    output_dir: str | Path,
    data_path: str | Path,
    assets: Mapping[str, ModAsset],
    active_ids: Sequence[str],
    ...
    inputs = build_dynamic_ror_compatibility_inputs(
        data_path,
        assets,
        active_ids,
        playset_id,
    ...
    fingerprint = fingerprint_dynamic_ror_compatibility_inputs(inputs)
    ...
    if not enabled:
        ...
        built: dict[str, Any] = {
            "path": "",
            "entry_count": 0,
            "stats": {"dynamic_ror_detected": 0, "patched_unit_count": 0},
        }
    else:
        if snapshot is None:
            raise ValueError("动态 RoR 兼容补丁数据源快照无效")
        for attempt in range(2):
            built = build_dynamic_ror_compatibility_patch(
                output_dir,
                str(data_path),
                dict(assets),
                list(active_ids),
            ...
            verified_inputs = build_dynamic_ror_compatibility_inputs(
                data_path,
                assets,
                active_ids,
                playset_id,
            ...
            verified_fingerprint = fingerprint_dynamic_ror_compatibility_inputs(
                verified_inputs
            )
            ...
            if attempt:
                raise ValueError(
                    "动态 RoR 兼容补丁的数据源在生成期间连续变化，请等待 MOD 更新完成后重试"
                )
            snapshot = verified_snapshot
            inputs = verified_inputs
            fingerprint = verified_fingerprint
        else:  # pragma: no cover
            raise ValueError("动态 RoR 兼容补丁生成未完成")
    ...
    if generated:
        ...
        if returned != patch_path.resolve(strict=False) or not patch_path.is_file():
            raise ValueError("动态 RoR 兼容补丁生成结果无效")
