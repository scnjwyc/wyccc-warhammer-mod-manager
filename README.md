...
一个面向所有支持 Steam 创意工坊的《全面战争》游戏的轻量 MOD 管理器，兼容 CA Pack 与 Rome Remastered 目录式 MOD。

当前版本为 `1.0.7`。它把 MOD、播放集、Steam 创意工坊与游戏启动集中在一个界面中，无需复杂配置即可开始使用。

## 当前功能

- 无需复杂配置，轻松上手。
- 默认选择《全面战争：战锤 3》；设置中也可切换战锤 1/2、三国、法老/王朝、特洛伊、大不列颠王座、阿提拉、罗马 2、幕府将军 2 和罗马重制版，并分别保存、自动检测游戏与创意工坊目录。
- 一目了然地管理已启用与未启用 MOD；支持 Ctrl 多选、Shift 连选、批量拖拽排序，以及在两个列表之间直接拖放。
- 支持 Ctrl+A 全选当前列表；搜索时只选择可见结果。Data 与创意工坊中的 MOD 增删可通过低开销文件事件即时同步，也可在设置中关闭。
- 自动排序，新手无需刻意关心MOD排序。
...
- Pack 游戏可直接启动游戏、继续最近的战役，或从存档列表选择指定存档载入。
- Pack 游戏可比较存档使用的 MOD 并一键启用。
- 《全面战争：战锤 3》支持导入原版 MOD 启动器预设，可快速新建播放集或覆盖当前播放集。
- 《全面战争：战锤 3》支持动态修改单位规模并同步调整阵型排深；普通单体巨兽可选择按血量或规模调整，并可动态去除单位与法术友军伤害，兼容所有 MOD 与版本更新。《全面战争：三国》支持单位数据修改，可调整单位人数、可安全推导的生命值、招募/维护费用、属性和武器弹丸数据；共享武器与弹丸会自动复制，避免影响未修改单位。三国不提供游戏数据修改、兼容补丁或原版播放集导入；其他游戏也不提供这些进阶功能。罗马重制版会验证 `modinfo.json`、`filelist.json` 和 `data` 目录，并通过官方 Feral MOD 管理器确认实际启用项与顺序，不写入未经公开验证的启用配置。
- 右键可复制 MOD 路径；Pack MOD 还可安全移入回收站。取消订阅会同步清理对应工坊目录，游戏运行期间会禁止这些文件操作。
- 游戏运行时自动进入静态低消耗模式，暂停扫描和目录监听，并在游戏退出后恢复主界面。
- 自动检查新版本，并在应用内查看更新日志、下载和安装更新。
- 内置简体中文、English、한국어、Русский、日本語和Español六种语言支持。
...
| 后端 | Python 标准库为主；负责 Steam 路径发现、Pack/目录式 MOD 扫描、加载清单、启动和本地持久化 |
...
- Node.js 22 或更高版本
- pnpm 11
- 已安装的任一受支持 Steam 版《全面战争》游戏，用于实际扫描和启动
...
PyInstaller 发布结果只包含 EXE；程序首次运行后会在 EXE 同目录创建 `data/` 保存便携数据，如果发布目录不可写则回退到上述用户目录。命令行 `--data-dir` 和环境变量 `WYCCC_MM_DATA_DIR` 可显式覆盖。升级安装会在新目录尚不存在时依次读取旧版 `%APPDATA%\WycccWarhammerManager\`、`%APPDATA%\WycccWarhammerModManager\`，避免丢失现有设置与播放集。建议将 EXE 放在独立程序文件夹中，再在桌面创建快捷方式；如果直接把 EXE 放在桌面，`data/` 也会按设计出现在桌面。
