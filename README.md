...
一个同时支持《全面战争：战锤 3》（Total War: WARHAMMER III）和《全面战争：三国》（Total War: THREE KINGDOMS）的轻量 MOD 管理器。

当前版本为 `0.8.2`。它把 MOD、播放集、Steam 创意工坊与游戏启动集中在一个界面中，无需复杂配置即可开始使用。

## 当前功能

- 无需复杂配置，轻松上手。
- 默认支持《全面战争：战锤 3》，也可在设置中切换到《全面战争：三国》，分别保存并自动检测游戏目录。
...
- 可比较存档使用的 MOD 并一键启用。
- 支持导入原版 MOD 启动器预设，可快速新建播放集或覆盖当前播放集。
- 支持动态修改单位规模并同步调整阵型排深；普通单体巨兽可选择按血量或规模调整，并可动态去除单位与法术友军伤害，兼容所有 MOD 与版本更新！
- 右键可复制 MOD 路径或将文件安全移入回收站；取消订阅会同步清理对应工坊目录。游戏运行期间会禁止这些文件操作。
...
- Node.js 22 或更高版本
- pnpm 11
- 已安装的 Steam 版《全面战争：战锤 3》或《全面战争：三国》用于实际扫描和启动
