...
const entries = {
  ...
  'gameData.intro': ['根据当前启用的 MOD 与原版 DB，点击“生成补丁”创建补丁；保存发生变化的设置时也会自动生成。', 'Use Generate patch to create a patch from the enabled MODs and vanilla DB; saving changed settings also generates it automatically.', '현재 활성화된 MOD와 기본 DB를 바탕으로 패치 생성 버튼을 눌러 패치를 만들 수 있으며, 변경된 설정을 저장할 때도 자동으로 생성됩니다.', 'Нажмите «Создать патч», чтобы создать его из активных MOD и исходной БД; при сохранении изменённых настроек он также создаётся автоматически.', '有効な MOD とバニラ DB から「パッチを生成」で作成でき、変更した設定を保存した場合も自動的に生成されます。'],
  ...
  'gameData.unitMultiplier': ['单位模型倍率', 'Unit model multiplier', '유닛 모델 수 배율', 'Множитель численности моделей', 'ユニットモデル数倍率'],
  'gameData.unitMultiplierHelp': ['调整非领主、非英雄单位的兵模人数，并同步骑乘与战争机器数量（0.5–5）；1 倍视为关闭此功能。', 'Scale model counts for non-lord, non-hero units together with mounts and engines (0.5–5); 1× disables this feature.', '군주와 영웅을 제외한 유닛의 모델 수와 탑승물 및 전쟁 기계 수를 함께 조정합니다(0.5–5). 1배에서는 이 기능이 꺼집니다.', 'Изменяет число моделей обычных отрядов, а также ездовых существ и машин (0,5–5); лорды и герои исключены. Значение 1× отключает эту функцию.', 'ロードとヒーローを除く兵数に加え、騎乗物と兵器数も同時に調整します（0.5～5）。1倍ではこの機能を無効として扱います。'],
  ...
  'gameData.runtimeNote': ['不会修改原始 Pack；补丁只会在手动生成或保存已变化设置时更新。', 'Original Packs are never modified; the patch is updated only when generated manually or when changed settings are saved.', '원본 Pack은 수정하지 않으며, 패치를 직접 생성하거나 변경된 설정을 저장할 때만 업데이트됩니다.', 'Исходные Pack не изменяются; патч обновляется только вручную или при сохранении изменённых настроек.', '元の Pack は変更せず、手動で生成した場合、または変更した設定を保存した場合のみパッチを更新します。'],
  ...
  'gameData.generatePatch': ['生成补丁', 'Generate patch', '패치 생성', 'Создать патч', 'パッチを生成'],
  'gameData.regenerateAfterModChanges': ['新增或删除涉及兵模数量或友伤的 MOD 后，必须重新生成补丁。', 'After adding or removing any MOD that affects model counts or friendly fire, you must generate the patch again.', '모델 수 또는 아군 피해에 영향을 주는 MOD를 추가하거나 제거한 뒤에는 반드시 패치를 다시 생성해야 합니다.', 'После добавления или удаления MOD, влияющего на численность моделей или урон союзникам, обязательно создайте патч заново.', '兵数または味方ダメージに影響する MOD を追加・削除した後は、必ずパッチを再生成してください。'],
  ...
  'busy.generateGameDataPatch': ['生成游戏数据补丁', 'Generating game data patch', '게임 데이터 패치 생성 중', 'Создание патча игровых данных', 'ゲームデータパッチを生成中'],
  ...
  'toast.gameDataPatchGenerated': ['游戏数据设置已保存，补丁已生成', 'Game data settings saved and patch generated', '게임 데이터 설정을 저장하고 패치를 생성했습니다', 'Настройки игровых данных сохранены, патч создан', 'ゲームデータ設定を保存し、パッチを生成しました'],
